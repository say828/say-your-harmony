...
function pairedTTest(baseline, comparison) {
...
function cohensD(baseline, comparison) {
...
// Analysis functions
function compareExperiments(baseline, comparison) {
    const calcPct = (base, comp) => ((base - comp) / base) * 100;
    return {
        baseline,
        comparison,
        deltas: {
...
async function analyzeScenario(resultsDir, scenario) {
    ...
    for (const [metricName, extractor] of Object.entries(metricExtractors)) {
        ...
        if (comparisonValues.length >= 1) {
            const { t, p } = pairedTTest(Array(comparisonValues.length).fill(extractor(baseline)), comparisonValues);
            const effectSize = cohensD(Array(comparisonValues.length).fill(extractor(baseline)), comparisonValues);
            statistics.push({
                metric: metricName,
                baselineValues,
                comparisonValues,
    ...
    // Summary
    const avgImprovement = mean(comparisons.map((c) => c.deltas.durationPct)) || 0;
...
function generateAnalysisReport(analysis) {
    ...
    analysis.comparisons.forEach((comp, idx) => {
        ...
        lines.push('|--------|----------|---------|--------|');
        lines.push(`| Duration | ${comp.baseline.totals.durationMin.toFixed(1)} min | ${comp.comparison.totals.durationMin.toFixed(1)} min | ${comp.deltas.durationPct >= 0 ? '↓' : '↑'} ${Math.abs(comp.deltas.durationPct).toFixed(1)}% |`);
        lines.push(`| Tool Calls | ${comp.baseline.totals.toolCalls} | ${comp.comparison.totals.toolCalls} | ${comp.deltas.toolCallsPct >= 0 ? '↓' : '↑'} ${Math.abs(comp.deltas.toolCallsPct).toFixed(1)}% |`);
        lines.push(`| Patterns | ${comp.baseline.totals.patterns} | ${comp.comparison.totals.patterns} | ${comp.deltas.patternsPct >= 0 ? '↓' : '↑'} ${Math.abs(comp.deltas.patternsPct).toFixed(1)}% |`);
        lines.push('');
    });
    ...
    analysis.statistics.forEach((stat) => {
        lines.push(`| ${stat.metric} | ${stat.mean.baseline.toFixed(1)} | ${stat.mean.comparison.toFixed(1)} | ${stat.tStatistic.toFixed(2)} | ${stat.pValue.toFixed(4)} | ${stat.significant ? '✅' : '❌'} | ${stat.effectSize.toFixed(2)} |`);
    });
    lines.push('');
