...
// Chart utilities
function barChart(data, width = 50) {
...
function lineChart(data, height = 10, width = 50) {
    const { labels, values, title, unit } = data;
    const lines = [];
    lines.push(title);
    lines.push('='.repeat(title.length));
...
function comparisonChart(baseline, comparison, labels, title, unit) {
    const lines = [];
    lines.push(title);
    lines.push('='.repeat(title.length));
    lines.push('');
...
function learningCurve(experiments, metric) {
...
// Main
async function visualizeExperiments(resultsDir, scenario) {
    ...
    if (experiments.length > 1) {
        ...
        sections.push(comparisonChart(baseline.totals.durationMin, others.map((e) => e.totals.durationMin), others.map((_, i) => `Run ${i + 2}`), 'Duration Comparison', 'min'));
        sections.push('');
        sections.push('');
        sections.push(comparisonChart(baseline.totals.toolCalls, others.map((e) => e.totals.toolCalls), others.map((_, i) => `Run ${i + 2}`), 'Tool Calls Comparison', 'calls'));
        sections.push('');
        sections.push('');
    }
    // Summary bar chart
