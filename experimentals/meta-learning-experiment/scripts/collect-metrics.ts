...
// Helper functions
function loadQuickMeta(metaDir: string, phase: string): QuickMeta | null {
  const filePath = path.join(metaDir, `${phase}.json`);
  try {
    const content = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
    return content as QuickMeta;
  } catch {
    return null;
  }
}

function calculatePhaseMetrics(meta: QuickMeta): PhaseMetricsExtended {
...
async function collectMetrics(
  sessionId: string,
  experimentId: string,
  baseDir: string = process.env.HOME + '/.claude/meta/quickmeta'
  ...
  const phaseMetrics: Record<string, PhaseMetricsExtended> = {};
  ...
  return {
    ...
    phases: phaseMetrics as any,
    totals,
    quality,
  };
}

async function loadQuickMetaAsync(sessionDir: string, phase: string): Promise<QuickMeta | null> {
  const filePath = path.join(sessionDir, `${phase}.json`);
  try {
    const content = await fs.readFile(filePath, 'utf-8');
    return JSON.parse(content) as QuickMeta;
  } catch {
    return null;
  }
}

function generateMarkdownReport(metrics: ExperimentMetrics): string {
  const lines: string[] = [
  ...
  for (const phase of phaseOrder) {
    const p = metrics.phases[phase as keyof typeof metrics.phases];
    if (p) {
...
`);
  ...
  for (let i = 0; i < args.length; i += 2) {
    ...
    else if (flag === '--base') baseDir = value;
  }
